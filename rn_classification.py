...
from calcule_cout_cee import *
from passe_avant import *
from passe_arriere import *
...
x, d, N, nb_var, nb_cible = lecture_donnees("TD6-donnees/car.txt")
# x, d, N, nb_var, nb_cible = lecture_donnees("TD6-donnees/iris.txt")
d = np.repeat(np.expand_dims(np.arange(d.max() + 1), axis=0), N, axis=0) == np.repeat(d, d.max()+1, axis=-1)
d = d.astype(int)
...
x_app, d_app, x_val, d_val, x_test, d_test = decoupage_donnees(x, d, prop_val=0.1, prop_test=0.1)
...
precisions_validation = np.zeros(nb_iters)

# Dimensions du réseau
nb_class = d_app.shape[1]
D_c = [nb_var, 5, nb_class]  # liste contenant le nombre de neurones pour chaque couche
activation = [sigmoide,
              softmax]  # liste contenant les fonctions d'activation des couches cachées et de la couche de sortie
...
for t in range(nb_iters):
    ...
    ########################################################################################
    # Calcul de la fonction perte de type CEE et la précision sur les données de validation#
    ########################################################################################
    couts_apprentissage[t] = calcule_cout_cee(y_app, d_app)
    couts_validation[t] = calcule_cout_cee(y_val, d_val)
    precisions_validation[t] = (y_val.argmax(axis=0) == d_val.argmax(axis=0)).sum() / y_val.shape[1]

    ####################################
    # Passe arrière : rétropropagation #
    ####################################
    delta_i = y_app - d_app  # Pour la dernière couche
    delta_W, delta_b = passe_arriere(delta_i, a, h, W, activation)
...
print("Précision final sur l'ensemble de validation : ", precisions_validation[-1])
...
plt.ylabel("Coût")
plt.figure(1)
plt.title("Evolution de la précision lors de la retropropagation")
plt.plot(precisions_validation)
plt.xlabel("Nombre d'iterations")
...
cout = calcule_cout_cee(y_test, d_test)
precision = (y_test.argmax(axis=0) == d_test.argmax(axis=0)).sum() / y_test.shape[1]
print("Coût sur l'ensemble de test : ", cout)
print("Précision de test :", precision)
